...
from .layers import BottleneckBlock, AdditiveAttention
...
# fmt: off
class MASSCAttention(ptl.LightningModule):
    ...
    def classify_segments(self, x, eval_frequency_sec=None):
        ...
        # Merge with attention mechanism
        if not self.testing:
            z = z_1.permute(0, 2, 1)
            z, alphas = self.attention(z, self.hparams.eval_frequency_sec)
    ...
    def test_epoch_end(self, output_results):
        ...
        alphas = torch.cat([out['alphas'] for out in output_results], dim=0)
...
if __name__ == "__main__":
    ...
    model = MASSCAttention(model_parameters)
