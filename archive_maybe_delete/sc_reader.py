...
class ScoreData:
    ...
    def report_cost(self, accuracy, loss, confidence):
        ...
        ind = np.arange(
            current_batch * self.config.batch_size, (current_batch + 1) * self.config.batch_size, step=1, dtype=np.int
        )
    ...
    def get_batch(self, batch_num):
        ...
        ind = np.arange(
            batch_num_ordered * self.batch_size, (batch_num_ordered + 1) * self.batch_size, step=1, dtype=np.int
        )
    ...
    def load(self):
        ...
        labels = np.reshape(
            labels, [self.config.segsize, self.n_seg, self.config.num_classes, labels.shape[2]], order="F"
        )
...
#             wr = csv.writer(csvfile, delimiter=",", quotechar="|", quoting=csv.QUOTE_NONE)
...
if __name__ == "__main__":
    ...
    config = sc_config.Config(parser)
    reader = ScoreData("/oak/stanford/groups/mignot/projects/sleep_staging/edf_encodings/train", config)
