...
def main(args):
    ...
    trainer = Trainer.from_argparse_args(
        args, deterministic=True, checkpoint_callback=checkpoint_logger, logger=wandb_logger, callbacks=[lr_logger]
    )
    ...
    results = trainer.test(test_dataloaders=khc_data, verbose=False)[0]
    evaluate_performance(results)
    print(len(results.keys()))
