...
def get_model(args):
    ...
    if args.model_type == "stages" and args.model_name.split("_")[1] == "rh":
    ...
    #         # model = models.MasscModel.load_from_checkpoint(args.resume_from_checkpoint)
    #         args = torch.load(args.resume_from_checkpoint, map_location=torch.device("cpu"))["hyper_parameters"]
    #         model = models.MasscV2Model(args)
    #     else:
    #         # model = models.MasscModel(**vars(args))
    #         model = models.MasscV2Model(vars(args))
    #     # model = models.MasscV2Model.load_fr1om_checkpoint(args.resume_from_checkpoint)
