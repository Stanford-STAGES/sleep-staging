...
def load_hypnogram_jcts(sta_file):
    ...
    if not os.path.exists(sta_file):
        return None
    with open(sta_file, "r") as fp:
        hyp = np.loadtxt(fp).astype(np.uint32)[:, 0, np.newaxis]
...
def load_hypnogram_khc(sta_file):
    ...
    try:
        ...
        sta_file = os.path.join('data', 'khc', 'hypnogram', sta_file)
...
def load_scored_data(sta_file, cohort=None):
