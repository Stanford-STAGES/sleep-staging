...
try:
    from sleep_staging.utils import ParallelExecutor, load_h5_data
except ImportError:
...
class KHCDataset(Dataset):
    ...
    def __init__(
        self, data_dir=None, encoding="raw", n_jobs=-1, n_records=-1, subset="test", overlap=True, adjustment=30, scaling=None,
    ):
        super().__init__()
...
class KHCDataModule(pl.LightningDataModule):
    def __init__(
        self, batch_size, data_dir=None, n_workers=0, n_jobs=-1, n_records=None, scaling="robust", adjustment=None, **kwargs,
    ):
        super().__init__()
        self.adjustment = adjustment
        self.data_dir = data_dir
