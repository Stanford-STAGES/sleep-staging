...
try:
    from sleep_staging.utils import load_h5_data
    from sleep_staging.utils import get_h5_info
    from sleep_staging.utils import ParallelExecutor
except ImportError:
    from sleep_staging.utils.h5_utils import get_h5_info
    from sleep_staging.utils.h5_utils import load_h5_data
    from sleep_staging.utils.parallel_bar import ParallelExecutor


class STAGESDataset(torch.utils.data.Dataset):
    def __init__(
        self, data_dir=None, batch_size=None, eval_ratio=None, n_classes=None, n_jobs=None, n_records=None, seg_size=None,
    ):
        super().__init__()
...
if __name__ == "__main__":
    ...
    dm_params = dict(
        batch_size=30, data_dir="data/ssc_wsc/cc/5min", eval_ratio=0.1, n_jobs=-1, n_records=100, n_workers=10, seg_size=60,
    )
