...
from sleep_staging.datasets import *


class AvgLossMasscModel(ptl.LightningModule):
    def __init__(
        self,
        filter_base=None,
        kernel_size=None,
  ...
  #          "f1": F1(reduce_op="mean"),
    ...
    def compute_loss(self, y, y_hat):
        ...
        loss = F.cross_entropy(y_hat.reshape(y_hat_shape[0], y_hat_shape[1], -1, 30).mean(dim=-1), y.argmax(dim=1)[:, ::30])
...
if __name__ == "__main__":
    ...
    model = AvgLossMasscModel(**model_parameters)
    model.configure_optimizers()
    model.setup("fit")
    model_summary = ptl.core.memory.ModelSummary(model, "full")
    print(model_summary)

    x, t, records, seq_nrs = next(iter(model.train_dataloader()))
    y = model(x)
    ...
    loss = model.compute_loss(torch.z, t)
