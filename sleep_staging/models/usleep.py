...
class USleepModel(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.use_ddp:
            ...
            s = stable_sleep.to(torch.bool).detach().cpu().numpy()
            u = t.sum(axis=-1) == 1

        else:
            t = true.detach().numpy()
            p = predicted.detach().numpy()
            s = stable_sleep.to(torch.bool).detach().numpy()
            u = t.sum(axis=-1) == 1
    ...
    def test_step(self, batch, batch_index):
        ...
        outputs = {
            f'yhat_{resolution}s': self.classify_segments(x, resolution)[0].softmax(1).squeeze(0).T.detach().cpu().numpy() for resolution in resolutions
        }
        outputs = {
    ...
    def add_model_specific_args(parent_parser):
        ...
        optimizer_group.add_argument('--lr_scheduler', type=str)
