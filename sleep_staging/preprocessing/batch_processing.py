...
from sleep_staging.preprocessing import process_single_file
from sleep_staging.utils.errors import MissingHypnogramError, MissingSignalsError, ReferencingError
...
def batch_chunk_wrapper(fs, seq_len, overlap, subset, encoding_type, out_dir, log_filename, cohort):
    import traceback
    ...
    for idx, fname in enumerate(tqdm(filelist)):
        ...
        try:
            ...
            if cohort in ["dcsm"]:
                save_name = os.path.join(save_dir, os.path.dirname(fname).split(os.path.sep)[-1] + ".h5")
            else:
...
if __name__ == "__main__":
    ...
    if args.mix:
        ...
        batch_chunk_wrapper(128, 10, 5, "train", "raw", "data/dcsm/raw", "logs/preprocessing/dcsm/raw/datasplit_0", "dcsm")
