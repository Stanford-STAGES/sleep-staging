...
def load_signals(edf_file, fs, cohort, encoding, channel_map_file) -> np.ndarray:
    ...
    #         if rem > 0:
    #             logging.info(
    #                 f"Trimming ch {idx} ({channel_labels[idx]}) from {len(data[idx])} to {len(data[idx]) - rem}"
    #             )
    ...
    if not all([isinstance(data[o], list) for o in occipital_list]):  # Some cohorts do not have occipital
        ...
        logging.info(f"Keeping {descr[keep_idx_occipital]} occipital EEG channel")
    else:
        keep_idx_occipital = None

    # Mar 6, 2023: Make record of all kept channels
    central_descr_dict = {"left": "C3", "right": "C4"}
    occipital_descr_dict = {"left": "O1", "right": "O2"}
    keep_dict = {"central_hemi": descr[keep_idx_central], "central_label": central_descr_dict[descr[keep_idx_central]]}
    if keep_idx_occipital:
        keep_dict["occipital_hemi"] = descr[keep_idx_occipital]
        keep_dict["occipital_label"] = occipital_descr_dict[descr[keep_idx_occipital]]
    else:
...
def trim_data_and_hypnogram(data, hypnogram, fs):
    t = 1 * fs
    t_30 = 30 * fs
    C, T = data.shape
    # logging.info(f"Trimming {hypnogram.shape[0] - T // t} epochs from hypnogram.")
    ...
    logging.info(f"Trimming {T - data.shape[1]} samples signal data from {T} to {data.shape[1]} samples")
    return data, hypnogram


def process_single_file(current_file, fs, seq_len, overlap, cohort, encoding="cc", channel_map_file=None):

    logging.info(f"Loading hypnogram for {current_file}")
