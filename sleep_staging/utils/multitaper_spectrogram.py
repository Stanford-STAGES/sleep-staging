...
def multitaper_spectrogram(
    ...
    [
        ...
        data, fs, frequency_range, time_bandwidth, num_tapers, window_params, min_nfft, detrend_opt, plot_on, verbose
    )
    ...
    if verbose:
        display_spectrogram_props(
            fs, time_bandwidth, num_tapers, [winsize_samples, winstep_samples], frequency_range, detrend_opt
        )
    ...
    calc_mts_segment_plus_args = partial(
        calc_mts_segment, DPSS_tapers=DPSS_tapers, nfft=nfft, freq_inds=freq_inds, detrend_opt=detrend_opt
    )
...
def process_input(
    ...
    if num_tapers != math.floor(2 * time_bandwidth) - 1:
        warnings.warn(
            "Number of tapers is optimal at floor(2*TW) - 1. consider using " + str(math.floor(2 * time_bandwidth) - 1)
        )
    ...
    if min_nfft == 0:  # avoid divide by zero error in np.log2(0)
        ...
        nfft = max(max(2 ** math.ceil(np.log2(abs(winsize_samples))), winsize_samples), 2 ** math.ceil(np.log2(abs(min_nfft))))
...
#     mts_kwargs = dict(dpss_tapers=dpss_tapers, nfft=nfft, freq_inds=freq_inds, detrend_opt=detrend_opt, num_tapers=num_tapers, dpss_eigen=dpss_eigen, weighting=weighting, wt=wt)
...
# #         mt_spectrogram = Parallel(n_jobs=cpus, prefer='threads')(
# #             delayed(calc_mts_segment_plus_args)(data_segments[i]) for i in range(data_segments.shape[0])
# #         )
# #         mt_spectrogram = Parallel(n_jobs=cpus, prefer='threads')(
# #             delayed(calc_mts_segment)(data_seg, **mts_kwargs) for data_seg in data_segments
# #         )
# #         print(mt_spectrogram)
# #         return
