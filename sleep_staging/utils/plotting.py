...
def view_record(
    ...
    im = current_ax.pcolormesh(
        np.array(spec_t).flatten(),
        np.array(spec_f).flatten(),
        2 * nanpow2db(np.abs(Zxx)).T,
        cmap="jet",
        shading="auto",
        vmin=vmin,
        vmax=vmax,
...
def extended_epoch_view(
    ...
    x = (x.transpose(1, 0, 2).reshape(K, -1).reshape(K, -1, int(interval * 60 * fs)).transpose(1, 0, 2))[seqs]  # (M, K, T)
    ...
    target = (target.transpose(1, 0).reshape(K, -1, int(2 * interval)).transpose(1, 2, 0))[seq_idx_vec]  # (interval, 1, K)
...
def plot_segment(x, target, pred):
    ...
    for n in range(hypnodensity.shape[0]):
        l.append(
            plt.fill_between(np.arange(hypnodensity.shape[1]), y_[n, :], y_[n + 1, :], edgecolor="face", facecolor=cmap[n, :])
        )
