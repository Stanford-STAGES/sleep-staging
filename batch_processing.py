...
def batch_start_jobs(args):
    ...
    for current_file in listF:
        ...
        if df.query(f'ID == "{current_fid}"').empty: # The subject is not in the overview file and is automatically added to the train files
...
#SBATCH --output="/home/users/alexno/sleep-staging/batch/logs/{subset}/{args.encoding_type}/{os.path.basename(current_file)}.out"
#SBATCH --error="/home/users/alexno/sleep-staging/batch/logs/{subset}/{args.encoding_type}/{os.path.basename(current_file)}.err"
##################################################
...
python -c 'from batch_processing import {process_fn.__name__}; {process_fn.__name__}("{current_file}", {fs}, {seq_len}, {overlap}, "{subset}")'
'''
